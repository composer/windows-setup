...
To compile the main composer.iss script you must create a file named "develop.iss"
...
Use the Install Online button from the Certum certificate management page (the other options only download
the public key) then go to Options/Advanced/Certificates page. Backup with a .p12 extension then click on
the backup to import the certificate into the Windows Certificate Store - save it as the Current User,
otherwise you will have to compile as an admin. Adding it to the certificate store allows you to use the
/a option in signtool, which finds the most suitable certificate (rather than specifying a file and
password in the command params).
