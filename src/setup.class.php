...
class ComposerSetup
{
    ...
    */
    protected function phpCheck()
    {
        ...
        if (version_compare(PHP_VERSION, self::CS_MIN_PHP, '<')) {
            $format = 'Your PHP (%s) is too old, you must upgrade to PHP %s or higher.';
            echo sprintf($format, PHP_VERSION, self::CS_MIN_PHP), PHP_EOL;
        ...
        if (!empty($this->errors)) {
            $this->outputPhpErrors();
        }
        $this->debug(empty($this->errors) ? 'success' : 'errors');
    ...
    */
    ...
    {
        $s = implode(PHP_EOL, array_map('trim', $this->errors));
        $this->errors = array();
    ...
    */
    ...
    {
        // strip out errors that include copy()
    ...
    * Formats any errors
    */
    protected function outputPhpErrors()
    {
        $list = array();

        foreach (array_keys($this->errors) as $error) {
    ...
    protected function setupTestPhp($before)
    {
        ...
        if ($before) {
            ...
            switch ($test) {
                ...
                case 'p5': // multiline, status 0
                    echo $this->getIndentity();
                    echo 'xxx' . PHP_EOL;
                    $this->status = 0;
                    break;

                case 'p6': // first line only, status 1
                    echo $this->getIndentity();
                    $this->status = 1;
                    break;
            }
